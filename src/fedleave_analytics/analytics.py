...
def _comp_lots(
    ...
    earned = [row for row in transactions if row.get("category") == "comp" and row.get("direction") == "earned"]
...
def analyze_leave_year(payload: dict[str, Any], today: date | None = None) -> dict[str, Any]:
    ...
    transactions = [
        row for row in transactions if (transaction_date := _date(row)) and start <= transaction_date <= end
    ]
    ...
    for category in visible_categories:
        for direction in ("earned", "used"):
            if direction == "earned" and category in {"annual", "sick"}:
                continue
            selected_rows = [
                row for row in transactions if row.get("category") == category and row.get("direction") == direction
            ]
    ...
    for month in month_starts:
        ...
        overtime_rows = [
            row for row in month_rows if row.get("category") == "overtime" and row.get("direction") == "worked"
        ]
        overtime_months.append(
            {"month": _month_label(month), "month_key": month_key, **_time_values(overtime_rows, today)}
        )

        earned_or_added = sum(
            _hours(row) for row in month_rows if row.get("direction") in BALANCE_INCREASING_DIRECTIONS
        )
        ...
        net_accumulation.append(
            {
                "month": _month_label(month),
                "month_key": month_key,
                "earned_or_added": earned_or_added,
                "used": used,
                "paid_out": paid_out,
                "forfeited": forfeited,
                "expired": expired,
        ...
        for category in visible_categories:
            ...
            if not category_rows:
                continue
            increase = sum(
                _hours(row) for row in category_rows if row.get("direction") in BALANCE_INCREASING_DIRECTIONS
            )
            decrease = sum(
                _hours(row) for row in category_rows if row.get("direction") in BALANCE_DECREASING_DIRECTIONS
            )
            net_accumulation_categories.append(
                {
                    "month": _month_label(month),
                    "category": category,
                    "earned_or_added": increase,
                    "decreased": decrease,
        ...
        comp_earned = sum(
            _hours(row) for row in month_rows if row.get("category") == "comp" and row.get("direction") == "earned"
        )
        credit_earned = sum(
            _hours(row)
        ...
        comp_monthly.append(
            {
                ...
                **{
                    direction: sum(
                        _hours(row)
                        for row in month_rows
                        if row.get("category") == "comp" and row.get("direction") == direction
                    )
    ...
    net_future = comp_increases["future_scheduled"] - sum(
        lifecycle_lookup[key]["future_scheduled"] for key in disposal_keys
    )
    lifecycle_rows.extend(
        [
            {
                "metric": "Net Comp Change",
                "through_today": net_today,
                "future_scheduled": net_future,
                "full_leave_year": net_today + net_future,
                "units": "hours",
            },
            {
                "metric": "Outstanding Comp",
                "through_today": starting_comp + net_today,
                "future_scheduled": net_future,
                "full_leave_year": starting_comp + net_today + net_future,
                "units": "hours",
            },
        ]
    )
    ...
    for direction, label in (
        ...
        values = _time_values(
            [row for row in transactions if row.get("category") == "credit" and row.get("direction") == direction],
            today,
        )
    ...
    lifecycle_rows.extend(
        [
            {
                "metric": "Net Credit Hours Change",
                "through_today": credit_net_today,
                "future_scheduled": credit_net_future,
                "full_leave_year": credit_net_today + credit_net_future,
                "units": "hours",
            },
            {
                "metric": "Outstanding Credit Hours",
                "through_today": credit_starting + credit_net_today,
                "future_scheduled": credit_net_future,
                "full_leave_year": credit_starting + credit_net_today + credit_net_future,
                "units": "hours",
            },
        ]
    )
    ...
    credit_worked_rows = [
        row for row in transactions if row.get("category") == "credit" and row.get("direction") == "worked"
    ]
    ...
    average_days = (
        None if allocated_hours <= EPSILON else sum(hours * days for hours, days in allocated_uses) / allocated_hours
    )
    open_lots = [
        row for row in lots if isinstance(row["remaining_today"], (int, float)) and row["remaining_today"] > EPSILON
    ]
    ...
    matured_summary = [
        {
            "matured_lots": len(matured_lots),
            "earned_hours": matured_earned,
    ...
    summary = [
        {
            "metric": "Total Leave Used or Scheduled",
            "value": total_absence,
            "units": "hours",
            "period_or_date": f"{start.isoformat()} to {end.isoformat()}",
            "basis": "Used transactions, including future scheduled leave",
        },
        {
            "metric": "Highest Leave-Use Month",
            "value": highest_month["full_leave_year"] if highest_month else None,
            "units": "hours",
            "period_or_date": highest_month["month"] if highest_month else "N/A",
            "basis": "Full leave year, including future scheduled leave",
        },
        {
            "metric": "Highest Leave-Use Pay Period",
            "value": highest_period["full_leave_year"] if highest_period else None,
            "units": "hours",
            "period_or_date": f"PP {highest_period['pay_period']}" if highest_period else "N/A",
            "basis": "Full leave year, including future scheduled leave",
        },
        {
            "metric": "Overtime Worked",
            "value": lifecycle_lookup[("overtime", "worked")]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Overtime worked transactions",
        },
        {
            "metric": "Comp Earned Instead of Paid Overtime",
            "value": comp_earned["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Comp earned transactions",
        },
        {
            "metric": "Comp Used",
            "value": lifecycle_lookup[("comp", "used")]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Comp used transactions",
        },
        {
            "metric": "Comp Paid Out",
            "value": lifecycle_lookup[("comp", "paid_out")]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Comp paid-out transactions",
        },
        {
            "metric": "Comp Forfeited",
            "value": lifecycle_lookup[("comp", "forfeited")]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Comp forfeited transactions",
        },
        {
            "metric": "Current Outstanding Comp",
            "value": starting_comp + net_today,
            "units": "hours",
            "period_or_date": today.isoformat(),
            "basis": "Opening balance plus earned, less dispositions through today",
        },
        {
            "metric": "Average Days Between Earning and Using Comp",
            "value": average_days,
            "units": "days",
            "period_or_date": "Through today",
            "basis": "Hour-weighted allocated comp usage",
        },
        {
            "metric": "Oldest Outstanding Comp Lot",
            "value": oldest_lot["remaining_today"] if oldest_lot else None,
            "units": "hours",
        ...
        {
            "metric": "Percentage Consumed Before Expiration",
            "value": consumed_percentage,
            "units": "percent",
            "period_or_date": "Matured lots",
            "basis": "Allocated use on or before expiration",
        },
        {
            "metric": "Credit Hours Earned or Worked",
            "value": credit_values["earned"]["full_leave_year"] + credit_values["worked"]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Credit earned and worked transactions",
        },
        {
            "metric": "Credit Hours Used",
            "value": credit_values["used"]["full_leave_year"],
            "units": "hours",
            "period_or_date": "Full leave year",
            "basis": "Credit used transactions",
        },
        {
            "metric": "Current Outstanding Credit Hours",
            "value": credit_starting + credit_net_today,
            "units": "hours",
            "period_or_date": today.isoformat(),
            "basis": "Opening balance plus earned or worked, less dispositions through today",
        },
    ]
    ...
    if starting_comp > EPSILON and not lots:
        warnings.append(
            {
                "severity": "Information",
                "area": "Comp opening balance",
                "date_or_lot": start.isoformat(),
